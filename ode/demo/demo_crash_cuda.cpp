...
#define DISABLE_THRESHOLD \
  0.008  // maximum velocity (squared) a body can have and be disabled
#define DISABLE_STEPS \
  10  // number of steps a box has to have been disable-able before it will be
      // disabled
...
static void start() {
  ...
  cuda_body = cuda_initBodiesOnDevice(bodies);
  b_buff = (dBodyID)malloc(sizeof(dxBody) * bodies);
}
...
void resetSimulation() {
  ...
  for (dReal x = 0.0; x < COLS * (LENGTH + RADIUS); x += LENGTH + RADIUS)
    for (dReal y = -((ROWS - 1) * (WIDTH / 2 + RADIUS));
         y <= ((ROWS - 1) * (WIDTH / 2 + RADIUS)); y += WIDTH + RADIUS * 2)
  ...
  for (dReal z = WBOXSIZE / 2.0; z <= WALLHEIGHT; z += WBOXSIZE) {
    offset = !offset;
    for (dReal y = (-WALLWIDTH + z) / 2; y <= (WALLWIDTH - z) / 2;
         y += WBOXSIZE) {
...
static void command(int cmd) {
  switch (cmd) {
    ...
    case 'F':
      ...
      break;
    case '+':
      dWorldSetAutoEnableDepthSF1(world,
                                  dWorldGetAutoEnableDepthSF1(world) + 1);
      break;
    case '-':
      dWorldSetAutoEnableDepthSF1(world,
                                  dWorldGetAutoEnableDepthSF1(world) - 1);
    ...
    case 'X': {
      ...
      dReal force = 10;
      dBodySetLinearVel(cannon_ball_body, force * R4[2], force * R4[6],
                        force * R4[10]);
...
static void simLoop(int pause) {
  ...
  if (!pause) {
    ...
    for (j = 0; j < joints; j++) {
      ...
      // dMessage (0,"curturn %e, turn %e, vel %e", curturn, turn,
      // (turn-curturn)*1.0);
...
#elif defined(CUDASTEP)
      cuda_copyBodiesToDevice(cuda_body, body, bodies);
      cuda_dxProcessIslands(world, cuda_body, 0.05, nullptr);
      cuda_copyBodiesFromDevice(body, cuda_body, bodies, b_buff);
#endif
    } else {
    ...
    for (i = 0; i < wb; i++) {
      ...
      if (dBodyIsEnabled(b)) {
        ...
        if (lspeed > DISABLE_THRESHOLD) disable = false;
        const dReal *avel = dBodyGetAngularVel(b);
        dReal aspeed =
            avel[0] * avel[0] + avel[1] * avel[1] + avel[2] * avel[2];
        if (aspeed > DISABLE_THRESHOLD) disable = false;
  ...
  dsSetColor(1, 1, 1);
  for (i = 0; i < spheres; i++)
    dsDrawSphere(dGeomGetPosition(sphere[i]), dGeomGetRotation(sphere[i]),
                 RADIUS);
  // sc.draw();
  // draw the cannon
  dsSetColor(1, 1, 0);
  ...
  dsDrawSphere(dBodyGetPosition(cannon_ball_body),
